...
//? Challenge: using array.protoytype.map(), console.log the first and last name of each president from the given array.

const fullName = presidents.map(president => `${president.first}${president.last}`);
console.log(fullName);

const aFullName = presidents.map((president,index) => `${president.first} ${president.last} ${index +1}`);
